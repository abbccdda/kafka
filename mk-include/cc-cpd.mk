...
_run-cc-system-tests: checkout-cc-system-tests cpd-deploy-local
	...
	set -o pipefail && $(CPD_PATH) priv testenv --id `kubectl config current-context` | tee ./.cc-system-tests/.profile
	@echo "## Running cc-system-tests's $(MAKE) init-env."
	source ./.cc-system-tests/.profile && $(MAKE) -C ./.cc-system-tests init-env
	...
	source ./.cc-system-tests/.profile && TEST_REPORT_FILE="$(BUILD_DIR)/ci-gating/TEST-report.xml" TESTS_TO_RUN='$(TESTS_TO_RUN)' $(MAKE) -C ./.cc-system-tests test
