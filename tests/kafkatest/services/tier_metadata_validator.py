...
"""
The tier storage metadata validator is a service which validates metadata integrity. Currently in downloads the tier
...
class TierMetadataValidator(KafkaPathResolverMixin, JmxMixin, BackgroundThreadService):
  ...
  def _worker(self, idx, node):
    self.security_config = self.kafka.security_config.client_config(node=node, jaas_override_variables=self.jaas_override_variables)
    ...
    for knode in self.kafka.nodes:
        ...
        for state_file in files:
            ...
            self.logger.info("Copying %s:%s to %s:%s", knode.account.hostname, state_file, node.account.hostname, dest_dir)
