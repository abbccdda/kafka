...
public class GoalViolationDetector implements Runnable {
  ...
  private void refreshBalancednessScore(Map<Boolean, List<String>> violatedGoalsByFixability) {
    double computedBalancednessScore = MAX_BALANCEDNESS_SCORE;
    for (List<String> violatedGoals : violatedGoalsByFixability.values()) {
      for (String violatedGoal : violatedGoals) {
        computedBalancednessScore -= _balancednessCostByGoal.get(violatedGoal);
      }
    }

    _balancednessScore = computedBalancednessScore;
  }
  ...
  private boolean optimizeForGoal(ClusterModel clusterModel,
    ...
    boolean skipReplicationFactorCheck = goal.canChangeReplicationFactor();
    try {
    ...
    Set<ExecutionProposal> proposals = AnalyzerUtils.getDiff(initReplicaDistribution, initLeaderDistribution,
            initObserverDistribution, clusterModel, skipReplicationFactorCheck);
