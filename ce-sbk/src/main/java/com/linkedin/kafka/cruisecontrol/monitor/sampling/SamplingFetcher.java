...
class SamplingFetcher extends MetricFetcher {
  ...
  protected void fetchMetricsForAssignedPartitions() throws MetricSamplingException {
    final TimerContext ctx = _fetchTimer.time();
  ...
  private void addPartitionSamples(Set<PartitionMetricSample> partitionMetricSamples) {
    ...
    if (partitionMetricSamples != null) {
      ...
      LOG.debug("Collected {} ({} discarded) partition metric samples for {} partitions. Total partition assigned: {}.",
                partitionMetricSamples.size(), discarded, returnedPartitions.size(), _assignedPartitions.size());
    } else {
  ...
  private void addBrokerMetricSamples(Set<BrokerMetricSample> brokerMetricSamples) {
    ...
    if (brokerMetricSamples != null) {
      ...
      LOG.debug("Collected {} ({} discarded) broker metric samples for {} brokers.",
                brokerMetricSamples.size(), discarded, returnedBrokerIds.size());
    } else {
