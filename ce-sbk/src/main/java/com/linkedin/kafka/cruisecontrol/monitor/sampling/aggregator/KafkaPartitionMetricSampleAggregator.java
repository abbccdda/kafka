...
public class KafkaPartitionMetricSampleAggregator extends MetricSampleAggregator<String, PartitionEntity> {
   ...
   * @param metadataClient A client to fetch metadata for the cluster.
   */
  ...
  private boolean isValidSample(PartitionMetricSample sample, boolean leaderValidation) {
    boolean validLeader = leaderValidation ? isValidLeader(sample) : true;
  ...
  public boolean isValidLeader(PartitionMetricSample sample) {
    Node leader = _metadataClient.cluster().leaderFor(sample.entity().tp());
    boolean validLeader = (leader != null) && (sample.brokerId() == leader.id());
    if (!validLeader) {
      LOG.warn("The metric sample will be discarded due to invalid leader. Current leader {}, Sample: {}", leader, sample);
    }
    return validLeader;
  }
