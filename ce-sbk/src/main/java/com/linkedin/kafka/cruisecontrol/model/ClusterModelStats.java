...
public class ClusterModelStats {
  ...
  private void utilizationForResources(ClusterModel clusterModel) {
    ...
    for (Resource resource : Resource.cachedValues()) {
      ...
      double balanceLowerThreshold = avgUtilizationPercentage * Math.max(0, 2.0 - _balancingConstraint.resourceBalancePercentage(resource));
  ...
  private void populateStatsForDisks(ClusterModel clusterModel, BalancingConstraint balancingConstraint) {
    ...
    for (Broker broker : clusterModel.aliveBrokers()) {
      ...
      double lowerLimit = brokerDiskUtilization * Math.max(0, 2.0 - balancingConstraint.resourceBalancePercentage(Resource.DISK));
