...
public class PartitionMetricSampleTest {
  ...
  public void testRecordAfterClose() {
    ...
    sample.close(0);
    try {
      sample.record(KafkaMetricDef.commonMetricDefInfo(KafkaMetricDef.DISK_USAGE), 0.0);
  ...
  public void testRecordSameResourceMetricAgain() {
    PartitionMetricSample sample = new PartitionMetricSample(0, new TopicPartition("topic", 0));
    sample.record(KafkaMetricDef.commonMetricDefInfo(KafkaMetricDef.DISK_USAGE), 0);
    try {
      sample.record(KafkaMetricDef.commonMetricDefInfo(KafkaMetricDef.DISK_USAGE), 0.0);
      fail("Should throw IllegalStateException");
    } catch (IllegalStateException ise) {
  ...
  public void testSerde() throws UnknownVersionException {
    ...
    sample.record(metricDef.metricInfo(KafkaMetricDef.PRODUCE_RATE.name()), (double) i++);
    sample.record(metricDef.metricInfo(KafkaMetricDef.FETCH_RATE.name()), (double) i++);
    sample.record(metricDef.metricInfo(KafkaMetricDef.MESSAGE_IN_RATE.name()), (double) i++);
    sample.record(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_IN_RATE.name()), (double) i++);
    sample.record(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_OUT_RATE.name()), (double) i);
    sample.close(10);
    ...
    assertEquals(sample.metricValue(metricDef.metricInfo(KafkaMetricDef.PRODUCE_RATE.name()).id()),
                 deserializedSample.metricValue(metricDef.metricInfo(KafkaMetricDef.PRODUCE_RATE.name()).id()), EPSILON);
    assertEquals(sample.metricValue(metricDef.metricInfo(KafkaMetricDef.FETCH_RATE.name()).id()),
                 deserializedSample.metricValue(metricDef.metricInfo(KafkaMetricDef.FETCH_RATE.name()).id()), EPSILON);
    assertEquals(sample.metricValue(metricDef.metricInfo(KafkaMetricDef.MESSAGE_IN_RATE.name()).id()),
                 deserializedSample.metricValue(metricDef.metricInfo(KafkaMetricDef.MESSAGE_IN_RATE.name()).id()), EPSILON);
    assertEquals(sample.metricValue(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_IN_RATE.name()).id()),
                 deserializedSample.metricValue(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_IN_RATE.name()).id()), EPSILON);
    assertEquals(sample.metricValue(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_OUT_RATE.name()).id()),
                 deserializedSample.metricValue(metricDef.metricInfo(KafkaMetricDef.REPLICATION_BYTES_OUT_RATE.name()).id()), EPSILON);
