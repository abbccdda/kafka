...
public class ExecutionTaskManagerTest {
  ...
  public void testStateChangeSequences() {
    ...
    testSequences.add(Arrays.asList(ExecutionTask.State.IN_PROGRESS, ExecutionTask.State.COMPLETED));
    // Rollback succeeded.
    testSequences.add(Arrays.asList(ExecutionTask.State.IN_PROGRESS, ExecutionTask.State.ABORTING, ExecutionTask.State.ABORTED));
    // Rollback failed.
    testSequences.add(Arrays.asList(ExecutionTask.State.IN_PROGRESS, ExecutionTask.State.ABORTING, ExecutionTask.State.DEAD));
    // Cannot rollback.
    testSequences.add(Arrays.asList(ExecutionTask.State.IN_PROGRESS, ExecutionTask.State.DEAD));
  ...
  private void changeTaskState(ExecutionTask.State state, ExecutionTask task, ExecutionTaskManager taskManager) {
    ...
    switch (state) {
      case IN_PROGRESS:
        ...
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.PENDING));
        assertEquals(1, (int) taskStat.get(ExecutionTask.State.IN_PROGRESS));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.ABORTING));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.ABORTED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.COMPLETED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.DEAD));
      ...
      case ABORTING:
        ...
        taskStat = executionTasksSummary.taskStat().get(ExecutionTask.TaskType.INTER_BROKER_REPLICA_ACTION);
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.PENDING));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.IN_PROGRESS));
        assertEquals(1, (int) taskStat.get(ExecutionTask.State.ABORTING));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.ABORTED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.COMPLETED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.DEAD));
        assertEquals(0, executionTasksSummary.remainingInterBrokerDataToMoveInMB());
      ...
      case DEAD:
        ...
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.ABORTED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.COMPLETED));
        assertEquals(1, (int) taskStat.get(ExecutionTask.State.DEAD));
        assertEquals(0, executionTasksSummary.remainingInterBrokerDataToMoveInMB());
      ...
      case COMPLETED:
        ...
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.IN_PROGRESS));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.ABORTING));
        assertEquals(origState == ExecutionTask.State.ABORTING ? 1 : 0, (int) taskStat.get(ExecutionTask.State.ABORTED));
        assertEquals(origState == ExecutionTask.State.ABORTING ? 0 : 1, (int) taskStat.get(ExecutionTask.State.COMPLETED));
        assertEquals(0, (int) taskStat.get(ExecutionTask.State.DEAD));
        assertEquals(0, executionTasksSummary.remainingInterBrokerDataToMoveInMB());
        assertEquals(0, executionTasksSummary.inExecutionInterBrokerDataMovementInMB());
        assertEquals(10, executionTasksSummary.finishedInterBrokerDataMovementInMB());
