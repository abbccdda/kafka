...
class OptimizationVerifier {
  ...
  /**
   * See {@link #executeGoalsFor(BalancingConstraint, ClusterModel, List, Collection, List, boolean, boolean)}.
   *
   * @param constraint         Balancing constraint for the given cluster.
   * @param clusterModel       The state of the cluster.
   * @param goalNameByPriority Name of goals by the order of execution priority.
  ...
  static boolean executeGoalsFor(BalancingConstraint constraint,
                                 ...
                                 List<Verification> verifications) throws Exception {
    return executeGoalsFor(constraint, clusterModel, goalNameByPriority, excludedTopics, verifications, false, false);
  }
  ...
  static boolean executeGoalsFor(BalancingConstraint constraint,
                                 ...
                                 boolean separateHardGoalsAndSoftGoals,
                                 boolean isTriggeredByGoalViolation) throws Exception {
    ...
    if (!separateHardGoalsAndSoftGoals) {
      ...
      resultOfFirstPass = goalOptimizer.optimizations(clusterModel, goalsOfFirstPass, new OperationProgress(), isTriggeredByGoalViolation);
    } else {
      ...
      resultOfFirstPass = goalsOfFirstPass.isEmpty() ? null
                                                     : goalOptimizer.optimizations(clusterModel, goalsOfFirstPass,
                                                                                   new OperationProgress(), isTriggeredByGoalViolation);
      ...
      resultOfSecondPass = goalsOfSecondPass.isEmpty() ? null
                                                       : goalOptimizer.optimizations(clusterModel, goalByPriority,
                                                                                     new OperationProgress(), isTriggeredByGoalViolation);
    }
    ...
    for (Verification verification : verifications) {
      switch (verification) {
        ...
        case NO_SOFT_GOAL_PROPOSALS:
          if (resultOfSecondPass != null && !verifyNoProposals(resultOfSecondPass)) {
            return false;
          }
          break;
        default:
  ...
  enum Verification {
    GOAL_VIOLATION, BROKEN_BROKERS, NEW_BROKERS, REGRESSION, NO_SOFT_GOAL_PROPOSALS
  }
}
