...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request, bufferSupplier: BufferSupplier): Unit = {
    try {
      ...
      request.header.apiKey match {
        ...
        case ApiKeys.REMOVE_BROKERS => handleRemoveBrokersRequest(request)
        case ApiKeys.DESCRIBE_BROKER_REMOVALS => handleDescribeBrokerRemovalsRequest(request)
    ...
    val (authorizedResources, unauthorizedResources) = alterConfigsRequest.configs.asScala.toMap.partition { case (resource, _) =>
      resource.`type` match {
        ...
        case ConfigResource.Type.CLUSTER_LINK =>
          throw new InvalidRequestException(s"Resource type CLUSTER_LINK not supported for alterConfigs()")
  ...
  private def configsAuthorizationApiError(resource: ConfigResource): ApiError = {
    val error = resource.`type` match {
      case ConfigResource.Type.BROKER | ConfigResource.Type.BROKER_LOGGER | ConfigResource.Type.CLUSTER_LINK => Errors.CLUSTER_AUTHORIZATION_FAILED
  ...
  def handleIncrementalAlterConfigsRequest(request: RequestChannel.Request): Unit = {
    ...
    val (authorizedResources, unauthorizedResources) = configs.partition { case (resource, _) =>
      resource.`type` match {
        case ConfigResource.Type.BROKER | ConfigResource.Type.BROKER_LOGGER | ConfigResource.Type.CLUSTER_LINK =>
          authorize(request.context, ALTER_CONFIGS, CLUSTER, CLUSTER_NAME)
        case ConfigResource.Type.TOPIC =>
          authorize(request.context, ALTER_CONFIGS, TOPIC, resource.name)
        case rt => throw new InvalidRequestException(s"Unexpected resource type $rt")
  ...
  def handleDescribeConfigsRequest(request: RequestChannel.Request): Unit = {
    ...
    }.toMap, describeConfigsRequest.includeSynonyms, request.session.principal, describeConfigsRequest.includeDocumentation)
