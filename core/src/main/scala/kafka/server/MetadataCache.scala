  ...
  private def getPartitionMetadata(snapshot: MetadataSnapshot, topic: String, listenerName: ListenerName, errorUnavailableEndpoints: Boolean,
    ...
    snapshot.partitionStates.get(topic).map { partitions =>
      partitions.map { case (partitionId, partitionState) =>
        ...
        val observers = partitionState.observers
        val filteredObservers = maybeFilterAliveReplicas(snapshot, observers, listenerName, errorUnavailableEndpoints)
        ...
        maybeLeader match {
          case None =>
            ...
            new MetadataResponse.PartitionMetadata(error, topicPartition, Optional.empty(),
              Optional.of(leaderEpoch), filteredReplicas, filteredObservers, filteredIsr, offlineReplicas)
          ...
          case Some(leader) =>
            if (filteredReplicas.size < replicas.size) {
              ...
              new MetadataResponse.PartitionMetadata(Errors.REPLICA_NOT_AVAILABLE, topicPartition,
                Optional.of(leader.id), Optional.of(leaderEpoch), filteredReplicas, filteredObservers, filteredIsr, offlineReplicas)
