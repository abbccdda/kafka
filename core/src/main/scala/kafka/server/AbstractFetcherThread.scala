...
/*
 The replica fetcher follows a state machine progression.
 In non-tiered mode, the fetcher is generally in a Fetching steady state, unless it hits some condition
 in which it needs to truncate, after which it moves back to a Fetching state.

 In tiered mode, two additional states are added: MaterializingTierMetadata and FetchingTierState.
 MaterializingTierMetadata starts a listener on the partition's TierPartitionState, which will be completed
 when the TierPartitionState has caught up to the leader's local dataset.

 Once the TierPartitionState has caught up, the PartitionState can move to FetchingTierState.
 In this state, a request is made to a tier state fetcher thread, to fetch the leader epoch state, and producer state.
 Upon retrieving and restoring this state and re-initializing the local log, the ReplicaFetcher can start Fetching
 from the leader.
...
abstract class AbstractFetcherThread(name: String,
  ...
  private def transitionMaterializingTierMetadata(topicPartition: TopicPartition,
    ...
    try {
      // We've found a tiered segment and tier state that aligns with the start of the leader's log. Next we will start
      // a fetch of the tier state from object storage.
      ...
      info(s"Restoring tiered state corresponding to $tierObjectMetadata for partition $topicPartition")
      partitionStates.update(topicPartition,
        currentFetchState.copy(state = FetchingTierState(completionStatus, tierObjectMetadata, currentFetchState.state)))
    } catch {
  ...
  private def transitionFetchingTierState(topicPartition: TopicPartition,
    ...
    // and producer state, and truncate the local log aligned to the end of the restored state.
    try {
      ...
      info(s"Tier state ${tierState.tierObjectMetadata} restored. Resuming replicating leader's local log " +
        s"from $proposedLocalLogStart.")
      partitionStates.update(topicPartition,
  ...
  protected def onOffsetTiered(topicPartition: TopicPartition, requestEpoch: Option[Int]): Boolean = {
    try {
      Option(partitionStates.stateValue(topicPartition)).foreach { currentFetchState =>
        ...
        info(s"Processed OFFSET_TIERED exception for $topicPartition. Waiting until tier partition state is " +
          s"materialized until ${leaderStartOffset - 1}")
        partitionStates.updateAndMoveToEnd(topicPartition,
          currentFetchState.copy(state = MaterializingTierMetadata(completionStatus, currentFetchState.state)))
      }
      true
    } catch {
  ...
  /**
    ...
    */
  protected def updateFetchOffsetAndMaybeMarkTruncationComplete(fetchOffsets: Map[TopicPartition, OffsetTruncationState]): Unit = {
