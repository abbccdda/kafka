...
class LogManager(logDirs: Seq[File],
  ...
  def truncateTo(partitionOffsets: Map[TopicPartition, Long], isFuture: Boolean) {
    ...
    for ((dir, _) <- affectedLogs.groupBy(_.dir.getParentFile)) {
      checkpointRecoveryOffsets(dir)
    }
  }
  ...
  def truncateFullyAndStartAt(topicPartition: TopicPartition, newOffset: Long, isFuture: Boolean) {
    ...
    if (log != null) {
      ...
      checkpointRecoveryOffsets(log.dir.getParentFile)
    }
  }
  ...
  def replaceCurrentWithFutureLog(topicPartition: TopicPartition): Unit = {
    logCreationOrDeletionLock synchronized {
      ...
      try {
        ...
        sourceLog.close()
        checkpointRecoveryOffsets(sourceLog.dir.getParentFile)
  ...
  def asyncDelete(topicPartition: TopicPartition, isFuture: Boolean = false): AbstractLog = {
    ...
    if (removedLog != null) {
      ...
      checkpointRecoveryOffsets(removedLog.dir.getParentFile)
