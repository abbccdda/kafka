...
class LogManager(logDirs: Seq[File],
  ...
  def truncateTo(partitionOffsets: Map[TopicPartition, Long], isFuture: Boolean): Unit = {
    ...
    for (logDir <- affectedLogs.map(_.parentDirFile).distinct) {
      checkpointRecoveryOffsets(logDir)
    }
  }
  ...
  def truncateFullyAndStartAt(topicPartition: TopicPartition, newOffset: Long, isFuture: Boolean): Unit = {
    ...
    if (log != null) {
      ...
      checkpointRecoveryOffsets(log.parentDirFile)
    }
  }
  ...
  /**
    ...
    */
  // Only for testing
  private[log] def checkpointRecoveryOffsets(dir: File): Unit = {
    try {
   ...
   */
  def getOrCreateLog(topicPartition: TopicPartition, loadConfig: () => LogConfig, isNew: Boolean = false, isFuture: Boolean = false): AbstractLog = {
    logCreationOrDeletionLock synchronized {
  ...
  def replaceCurrentWithFutureLog(topicPartition: TopicPartition): Unit = {
    logCreationOrDeletionLock synchronized {
      ...
      try {
        ...
        sourceLog.close()
        checkpointRecoveryOffsets(sourceLog.parentDirFile)
  ...
  def asyncDelete(topicPartition: TopicPartition, isFuture: Boolean = false): AbstractLog = {
    ...
    if (removedLog != null) {
      ...
      checkpointRecoveryOffsets(removedLog.parentDirFile)
