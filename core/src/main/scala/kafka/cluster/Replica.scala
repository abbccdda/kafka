...
class Replica(val brokerId: Int,
  ...
  def maybeFetchHighWatermarkOffsetMetadata(): Unit = {
    ...
    if (highWatermarkMetadata.messageOffsetOnly) {
      highWatermarkMetadata = log.get.convertToLocalOffsetMetadata(highWatermark).getOrElse {
        log.get.convertToLocalOffsetMetadata(logStartOffset).getOrElse {
          val firstSegmentOffset = log.get.localLogSegments.head.baseOffset
