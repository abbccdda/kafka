...
/**
 * An input stream wrapper which decides to close or abort an InputStream based on
 * the remaining data left in the stream. This approach is adapted from Hadoop's S3a logic.
 */
public class AutoAbortingGenericInputStream extends InputStream {
    ...
    AutoAbortingGenericInputStream(InputStream innerInputStream,
                                   long autoAbortSize,
                                   long totalBytes) {
    ...
    public void close() {
        ...
        boolean shouldDrain = !exception && remainingBytes() < autoAbortSize;
        try {
            if (shouldDrain) {
