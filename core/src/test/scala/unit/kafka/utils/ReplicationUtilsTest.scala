...
class ReplicationUtilsTest extends ZooKeeperTestHarness {
  ...
  override def setUp(): Unit = {
    ...
    val leaderAndIsr = LeaderAndIsr(leader, leaderEpoch, isr, 1, isUnclean = false, None)
  ...
  def testUpdateLeaderAndIsr(): Unit = {
    ...
    // regular update
    val newLeaderAndIsr1 = new LeaderAndIsr(leader, leaderEpoch, replicas, 0, false, None)
    ...
    val newLeaderAndIsr2 = new LeaderAndIsr(leader, leaderEpoch, replicas, zkVersion + 1, false, None)
    ...
    val newLeaderAndIsr3 = new LeaderAndIsr(leader, leaderEpoch + 1, replicas, zkVersion + 1, false, None)
