...
class ReplicaAlterLogDirsThreadTest {
  ...
  def shouldUpdateLeaderEpochAfterFencedEpochError(): Unit = {
    ...
    val replicaManager = Mockito.mock(classOf[ReplicaManager])
    val quotaManager = Mockito.mock(classOf[ReplicationQuotaManager])
    val futureLog = Mockito.mock(classOf[AbstractLog])
  ...
  def shouldTruncateToReplicaOffset(): Unit = {
    ...
    val logT1p0: AbstractLog = createNiceMock(classOf[AbstractLog])
    val logT1p1: AbstractLog = createNiceMock(classOf[AbstractLog])
    // one future replica mock because our mocking methods return same values for both future replicas
    val futureLogT1p0: AbstractLog = createNiceMock(classOf[AbstractLog])
    val futureLogT1p1: AbstractLog = createNiceMock(classOf[AbstractLog])
    val partitionT1p0: Partition = createMock(classOf[Partition])
    val partitionT1p1: Partition = createMock(classOf[Partition])
    val replicaManager: ReplicaManager = createMock(classOf[ReplicaManager])
  ...
  def shouldTruncateToEndOffsetOfLargestCommonEpoch(): Unit = {
    ...
    // one future replica mock because our mocking methods return same values for both future replicas
    val futureLog: AbstractLog = createNiceMock(classOf[AbstractLog])
  ...
  def stub(logT1p0: AbstractLog, logT1p1: AbstractLog, futureLog: AbstractLog, partition: Partition,
  ...
  def stubWithFetchMessages(logT1p0: AbstractLog, logT1p1: AbstractLog, futureLog: AbstractLog, partition: Partition, replicaManager: ReplicaManager,
