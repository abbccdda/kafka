...
class LogTest {
  ...
  def testLogSegmentsCallCorrect(): Unit = {
    ...
    def getSegmentOffsets(log :AbstractLog, from: Long, to: Long) = log.localLogSegments(from, to).map { _.baseOffset }
    ...
    // from < to is bad
    assertThrows[IllegalArgumentException]({ log.localLogSegments(10, 0) })
  }
  ...
  def testRecoverAfterNonMonotonicCoordinatorEpochWrite(): Unit = {
    ...
    var log = createLog(logDir, logConfig, maxProducerIdExpirationMs = maxProducerIdExpirationMs)
    val epoch = 0.toShort
  ...
  def testNonActiveSegmentsFrom(): Unit = {
    ...
    def nonActiveBaseOffsetsFrom(startOffset: Long): Seq[Long] = {
      log.localNonActiveLogSegmentsFrom(startOffset).map(_.baseOffset).toSeq
    }
  ...
  def testProducerExpireCheckAfterDelete(): Unit = {
    ...
    try {
      ...
      val producerExpireCheck = log.asInstanceOf[MergedLog].localLog.producerExpireCheck
