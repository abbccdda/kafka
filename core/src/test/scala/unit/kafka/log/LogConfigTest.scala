...
class LogConfigTest {
  ...
  def testFromPropsInvalid(): Unit = {
    LogConfig.configNames.foreach(name => name match {
      ...
      case LogConfig.TierSegmentHotsetRollMinBytesProp => assertPropertyInvalid(name, values = "not_a_number", "-0.1", "1.2")
      case LogConfig.PreferTierFetchMsProp => assertPropertyInvalid(name, values = "not_a_number", "-0.1", "1.2")
