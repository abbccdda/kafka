...
class LogCleanerIntegrationTest extends AbstractLogCleanerIntegrationTest with KafkaMetricsGroup {
  ...
  def testMarksPartitionsAsOfflineAndPopulatesUncleanableMetrics(): Unit = {
    ...
    val expectedTotalUncleanableBytes = LogCleanerManager.calculateCleanableBytes(log, 0, log.localLogSegments.last.baseOffset)._2 +
      LogCleanerManager.calculateCleanableBytes(log2, 0, log2.localLogSegments.last.baseOffset)._2
