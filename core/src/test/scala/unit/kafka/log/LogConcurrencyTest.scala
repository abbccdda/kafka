...
class LogConcurrencyTest {
  ...
  def testUncommittedDataNotConsumed(log: AbstractLog): Unit = {
   ...
   */
  private class ConsumerTask(log: AbstractLog, lastOffset: Int) extends Callable[Unit] {
    val consumedBatches = ListBuffer.empty[FetchedBatch]

    override def call(): Unit = {
      var fetchOffset = 0L
      while (log.highWatermark < lastOffset) {
        val readInfo = log.readLocal(
   ...
   */
  private class LogAppendTask(log: AbstractLog, lastOffset: Long) extends Callable[Unit] {
    override def call(): Unit = {
      var leaderEpoch = 1
      var isLeader = true
  ...
  private def createLog(config: LogConfig = LogConfig(new Properties())): AbstractLog = {
    Log(dir = logDir,
      config = config,
      logStartOffset = 0L,
      recoveryPoint = 0L,
  ...
  private def validateConsumedData(log: AbstractLog, consumedBatches: Iterable[FetchedBatch]): Unit = {
