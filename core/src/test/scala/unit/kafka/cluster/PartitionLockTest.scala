...
class PartitionLockTest extends Logging {
  ...
  private def setupPartitionWithMocks(logManager: LogManager, logConfig: LogConfig): Partition = {
    ...
    val partition = new Partition(topicPartition,
      ...
      override def createLog(replicaId: Int, isNew: Boolean, isFutureReplica: Boolean, offsetCheckpoints: OffsetCheckpoints): AbstractLog = {
        val log = super.createLog(replicaId, isNew, isFutureReplica, offsetCheckpoints)
        new LogUtils.SlowAppendAsLeaderLog(log.asInstanceOf[MergedLog], tierLogComponents, appendSemaphore)
      }
    }
