...
class AuthorizerIntegrationTest extends BaseRequestTest {
  ...
  def testUnauthorizedReplicaStatusWithoutDescribe(): Unit = {
    ...
    assertEquals(Errors.TOPIC_AUTHORIZATION_FAILED, Errors.forCode(replicaStatusResponse.data.topics.get(0).partitions.get(0).errorCode))
  }

  @Test
  def testUnauthorizedReplicaStatusWithDescribe(): Unit = {
    val authorizedTopic = "authorized-topic"
    createTopic(authorizedTopic)
    createTopic(topic)
    // Add ACLs to a different topic than the one used in the replica status request
    addAndVerifyAcls(Set(new AccessControlEntry(clientPrincipalString, WildcardHost, DESCRIBE, ALLOW)),
      new ResourcePattern(TOPIC, authorizedTopic, LITERAL))
    val replicaStatusResponse = connectAndReceive[ReplicaStatusResponse](replicaStatusRequest)
    assertEquals(Errors.TOPIC_AUTHORIZATION_FAILED, Errors.forCode(replicaStatusResponse.data.topics.get(0).partitions.get(0).errorCode))
  }
